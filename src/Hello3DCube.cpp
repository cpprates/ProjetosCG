...
//#endif
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::mat4(1); 
		model = glm::translate(model, glm::vec3(posX, posY, posZ)); // Translação do cubo
		model = glm::scale(model, glm::vec3(scaleFactor)); // Escala do cubo
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
    ...
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
		...
		float scaleSpeed = 0.1f; // fator de escala do cubo

        if (key == GLFW_KEY_W)
            posZ += moveSpeed;  // trás
        if (key == GLFW_KEY_S)
            posZ -= moveSpeed;  // frente
        if (key == GLFW_KEY_A)
		...
		if (key == GLFW_KEY_P)
			scaleFactor -= scaleSpeed; // Tecla P diminui a escala
		if (key == GLFW_KEY_O)
			scaleFactor += scaleSpeed; // Tecla P aumenta a escala
    }
